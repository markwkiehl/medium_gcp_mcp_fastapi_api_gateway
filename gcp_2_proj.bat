...
rem v0.0.1	Moved gcloud projects list to later in the script to avoid prompting to enable the Cloud Resource Manager API.
rem v0.0.2	Several revisions.  Now it runs without issues. 
rem v0.0.3	Deleted call "CALL gcloud config list" before authentication.
rem			Moved "gcloud services enable cloudresourcemanager.googleapis.com --project=%GCP_PROJ_ID%" ahead to avoid Y/N prompt.
