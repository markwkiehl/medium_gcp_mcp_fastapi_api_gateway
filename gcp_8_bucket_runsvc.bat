@echo off
cls
echo %~n0%~x0   version 0.0.3
echo.


rem v0.0.1	Added --timeout=360s --startup-cpu-boost  to cloud run deploy
rem			Assign the GCP_RUN_JOB_VOL_MT_PATH to the OS environment variable MOUNT_PATH
rem			Moved all contents from gcloud run services update to gcloud run services deploy. 
rem v0.0.2	Revised batch file sequence from 4 through 8
rem v0.0.3	Added call to gcloud run services logs read
...
rem The argument --allow-unauthenticated makes the Cloud Run publicly accessible.
rem Defaults are: --cpu=1  --memory=512 MiB
rem For 4.0 CPU, memory must be between 2Gi and 16Gi inclusive.
rem The default Cloud Run startup timeout is 240 seconds (4 minutes).
rem Cloud Run's --cpu-boost gives the container extra CPU during startup to reduce cold-start time.
rem gcloud run deploy [run service name] --image=[docker image name] --region=[region] --allow-unauthenticated
rem --no-allow-unauthenticated		--allow-unauthenticated
rem CALL gcloud run deploy %GCP_RUN_JOB% --image=%GCP_REGION%-docker.pkg.dev/%GCP_PROJ_ID%/%GCP_REPOSITORY%/%GCP_IMAGE%:latest --region=%GCP_REGION% --cpu=4 --memory=8Gi --concurrency=80 --allow-unauthenticated
rem CALL gcloud run deploy %GCP_RUN_JOB% --image=%GCP_REGION%-docker.pkg.dev/%GCP_PROJ_ID%/%GCP_REPOSITORY%/%GCP_IMAGE%:latest --region=%GCP_REGION% --allow-unauthenticated
rem CALL gcloud run deploy %GCP_RUN_JOB% --image=%GCP_REGION%-docker.pkg.dev/%GCP_PROJ_ID%/%GCP_REPOSITORY%/%GCP_IMAGE%:latest --region=%GCP_REGION% --allow-unauthenticated --cpu=4 --memory=3Gi --concurrency=80
rem CALL gcloud run deploy %GCP_RUN_JOB% --image=%GCP_REGION%-docker.pkg.dev/%GCP_PROJ_ID%/%GCP_REPOSITORY%/%GCP_IMAGE%:latest --region=%GCP_REGION% --allow-unauthenticated
rem v0.0.1 added: --timeout=300s --cpu-boost --startup-probe
@echo on
CALL gcloud run deploy %GCP_RUN_JOB% --image=%GCP_REGION%-docker.pkg.dev/%GCP_PROJ_ID%/%GCP_REPOSITORY%/%GCP_IMAGE%:latest --region=%GCP_REGION% --allow-unauthenticated --timeout=300s --cpu-boost --startup-probe=httpGet.port=8080,httpGet.path=/ready,initialDelaySeconds=10,failureThreshold=15,periodSeconds=20,timeoutSeconds=5 --add-volume name=%GCP_RUN_JOB_VOL_NAME%,type=cloud-storage,bucket=%GCP_GS_BUCKET% --add-volume-mount volume=%GCP_RUN_JOB_VOL_NAME%,mount-path=%GCP_RUN_JOB_VOL_MT_PATH% --region=%GCP_REGION% --set-env-vars=MOUNT_PATH=%GCP_RUN_JOB_VOL_MT_PATH%
@echo off
IF %ERRORLEVEL% NEQ 0 (
...
@echo off
echo.
echo You can access the app running as a Cloud Run Service by copying and pasting
...
echo .
rem gcloud run services describe YOUR_SERVICE_NAME --region YOUR_REGION
@echo on
CALL gcloud run services describe %GCP_RUN_JOB% --region=%GCP_REGION%
@echo off
...
echo .
echo Cloud Run Log:
Call gcloud run services logs read %GCP_RUN_JOB% --region=%GCP_REGION% --project=%GCP_PROJ_ID% --limit 50
echo .
...
echo.
echo When you are finished with the project, execute the batch file "gcp_cleanup".
