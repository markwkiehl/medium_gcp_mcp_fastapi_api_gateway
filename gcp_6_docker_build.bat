...
echo.
echo After the build, you can locally test running the Docker image with one of the following commands:
...
echo.
echo After this batch file %~n0%~x0 has ended normally (no errors), execute the batch file "gcp_7_docker_push".
pause

rem Build the Docker image %GCP_IMAGE%
rem Docker doesn't script well (often exits with a non-zero error code) and causes termination of the batch file.
...
rem --no-cache argument forces Docker to re-run every step, including the final CMD layer, ensuring your new instruction is incorporated into the image
...
@echo on
CALL docker build --no-cache --build-arg PY_VER=%GCP_PYTHON_VERSION% -t %GCP_IMAGE% .
@echo off
