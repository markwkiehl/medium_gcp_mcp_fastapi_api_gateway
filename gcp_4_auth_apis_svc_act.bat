...
rem v0.0.1	Revised gcloud storage buckets add-iam-policy-binding  ... --role=roles/storage.objectAdmin
...
rem gcloud storage buckets add-iam-policy-binding gs://mcp-search-docs-web-v0-2  --member=serviceAccount:svc-act-mcp-search-docs-web@mcp-search-docs-web-v0-2.iam.gserviceaccount.com --role=roles/storage.objectAdmin
@echo on
CALL gcloud storage buckets add-iam-policy-binding gs://%GCP_GS_BUCKET% --project=%GCP_PROJ_ID% --member=serviceAccount:%GCP_SVC_ACT% --role=roles/storage.objectAdmin
@echo off
IF %ERRORLEVEL% NEQ 0 (
...
rem -------------------------------------------------------------------------------------
rem Grant permissions to specific people by gmail address for the Google Cloud Run Service.
rem a6.024x23@gmail.com
rem vmoskal@smartcostinc.com
rem rich@marando-inc.com
rem gcloud run services add-iam-policy-binding YOUR_SERVICE_NAME --region=%GCP_REGION% --member=user:USER_GMAIL_ADDRESS --role=roles/run.invoker
...
echo Next, execute the batch file "gcp_5_copy_files_to_bucket".
